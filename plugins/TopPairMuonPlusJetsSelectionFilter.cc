...
bool TopPairMuonPlusJetsSelectionFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup, std::string MCSampleTag) {
	setupEventContent(iEvent, MCSampleTag);
	...
	for (unsigned int step = 0; step < TTbarMuPlusJetsReferenceSelection::NUMBER_OF_SELECTION_STEPS; ++step) {
		...
		if (debug_)
			cout << "Doing selection step: " << TTbarMuPlusJetsReferenceSelection::StringSteps[stepName] << " " << passesSelectionStep(iEvent, step, MCSampleTag) << endl;

		bool passesStep(passesSelectionStep(iEvent, step, MCSampleTag));
...
void TopPairMuonPlusJetsSelectionFilter::setupEventContent(edm::Event& iEvent, std::string MCSampleTag) {
...
void TopPairMuonPlusJetsSelectionFilter::cleanedJets(std::string MCSampleTag) {
	cleanedJets_.clear();
	for (unsigned index = 0; index < jets_.size(); ++index) {
		const pat::Jet jet = jets_.at(index);
		if (!isGoodJet(jet, MCSampleTag))
			continue;
		bool overlaps(false);
...
bool TopPairMuonPlusJetsSelectionFilter::isGoodJet(const pat::Jet& jet, std::string MCSampleTag) const {
	//both cuts are done at PAT config level (selectedPATJets) this is just for safety
	double smearFactor = getSmearedJetPtScale(jet, 0, MCSampleTag);
...
bool TopPairMuonPlusJetsSelectionFilter::passesSelectionStep(edm::Event& iEvent, unsigned int selectionStep, std::string MCSampleTag) const {
...
bool TopPairMuonPlusJetsSelectionFilter::hasAtLeastOneGoodJet(std::string MCSampleTag) const {
	if (cleanedJets_.size() > 0)		
		return getSmearedJetPtScale(cleanedJets_.at(0), 0, MCSampleTag)*cleanedJets_.at(0).pt() > min1JetPt_;
	return false;

}

bool TopPairMuonPlusJetsSelectionFilter::hasAtLeastTwoGoodJets(std::string MCSampleTag) const {
	if (cleanedJets_.size() > 1)
		return getSmearedJetPtScale(cleanedJets_.at(1), 0, MCSampleTag)*cleanedJets_.at(1).pt() > min2JetPt_;
	return false;
}

bool TopPairMuonPlusJetsSelectionFilter::hasAtLeastThreeGoodJets(std::string MCSampleTag) const {
	if (cleanedJets_.size() > 2)
		return getSmearedJetPtScale(cleanedJets_.at(2), 0, MCSampleTag)*cleanedJets_.at(2).pt() > min3JetPt_;
	return false;
}

bool TopPairMuonPlusJetsSelectionFilter::hasAtLeastFourGoodJets(std::string MCSampleTag) const {
	if (cleanedJets_.size() > 3)
		return getSmearedJetPtScale(cleanedJets_.at(3), 0, MCSampleTag)*cleanedJets_.at(3).pt() > min4JetPt_;
	return false;
}
